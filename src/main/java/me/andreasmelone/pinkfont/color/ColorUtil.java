...
public class ColorUtil {
    ...
    public static MutableComponent rainbowify(Component component, int factor) {
        ...
        for (Component c : component.toFlatList()) {
            ...
            for(int i = 0; i < content.length(); i++) {
                ...
                mutable.append(Component.literal(content.charAt(i) + "").withStyle(c.getStyle()).withStyle(style -> style.withColor(color)));
            }
    ...
    public static int floatToARGB(float a, float r, float g, float b) {
        int ia = Math.round(clamp01(a) * 255.0f);
